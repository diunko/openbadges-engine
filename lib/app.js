...
OpenBadges.initialize(badgesStorageConfig)
    .then(function (openBadges) {
        ...
        var storage = new Storage({ locator: argv.locator, namespace: 'OpenBadges', debug: true });

        storage.connect();

        storage.on('connect', function () {
            //storage.write('cookies', JSON.stringify([]), function (err) {});
            storage.write('openBadges', JSON.stringify(openBadges), function (err) {
